...
import { necklacePreviews } from "@/lib/necklaces";
import {
  MATERIAL_FILTERS,
  formatPrice,
  getProducts,
...
const collection = collectionJourney.find((c) => c.slug === "necklaces")!;

export const metadata: Metadata = {
  title: "Necklaces Collection — Kundan",
...
export default async function NecklacesPage({ searchParams }: PageProps) {
        ...
        <div id="shop" className="container-luxury scroll-mt-28 py-16 md:py-24">
          ...
          <div className="mb-10 flex flex-col justify-between gap-4 border-b border-border pb-6 sm:flex-row sm:items-end">
            ...
            <p className="text-[11px] tracking-[0.18em] text-muted uppercase">
              {necklaces.length} pieces · Ready to ship
            </p>
          </div>
          ...
          <div className="mb-8 flex flex-wrap gap-2">
            {MATERIAL_FILTERS.map((filter) => {
              ...
              const href =
                filter.value === "all"
                  ? "/collections/necklaces#shop"
                  : `/collections/necklaces?material=${filter.value}#shop`;
